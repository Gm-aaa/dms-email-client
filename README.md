...
- **高性能 Rust 后端**：
  - 采用基于事件驱动的 IMAP IDLE（推送通知）长连接监听新邮件，资源占用极低；服务器推送到达即刻唤醒同步。
  - **轮询兜底 + 保活**：部分服务商的 IDLE 不下发新邮件通知（如 QQ 邮箱），或经代理/NAT 的长连接会被静默回收，纯靠推送会漏。为此每挂 IDLE 至多 `poll_interval_secs`（默认 45 秒）就主动复查一遍：既把新邮件的最坏延迟兜在该值内，又用周期性小流量保活连接。IDLE 推送正常时仍近乎实时。
  - **零轮询状态推送**：前端通过长连接订阅守护进程状态，新邮件 / 已读变化由守护进程主动推送到界面（前端不做定时轮询），后端到界面近乎零延迟；空闲时几乎不产生唤醒。
...
# 不影响后台常驻等待新邮件（IDLE 的超时由 poll_interval_secs 决定）。
imap_timeout_secs = 60

# IDLE 轮询间隔（秒，取值 15–1740）。每挂 IDLE 至多这么久就主动复查一遍收件箱。
# 作用：(1) 兜底——某些服务商 IDLE 不推送新邮件通知（如 QQ 邮箱）、或经代理/NAT 的长连接
# 被静默回收时，靠这次周期性复查发现新邮件，最坏延迟 ≈ 本值；(2) 保活——周期性小流量
# 防止空闲长连接被代理/NAT 中途掐断。IDLE 推送正常送达时仍会提前唤醒，保持近乎实时。
...
- **导入并保存配置（通过标准输入接收 JSON）**：
  ```bash
  echo '{"cache_limit":50,"cache_dir":"...","body_cache_limit":500,"imap_timeout_secs":60,"poll_interval_secs":45,"accounts":[]}' | dms-email-client config save
  ```
