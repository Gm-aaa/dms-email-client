...
### 新增
- 配置项 `poll_interval_secs`（默认 45，取值 15–1740）：IDLE 轮询间隔，接管原先写死的 5 分钟
  IDLE 超时。每挂 IDLE 至多该值就主动复查一遍收件箱。

### 修复（实时性）
- 修复部分服务商新邮件严重延迟：原先实时性完全依赖 IMAP IDLE 推送。实测 **QQ 邮箱
  `imap.qq.com` 的 IDLE 根本不下发新邮件（EXISTS）通知**，且经代理/NAT 的长 IDLE 连接会在
  空闲约 2–3 分钟后被静默掐断——两种情况下纯靠推送都会漏，新邮件只能等到 5 分钟超时才出现。
  改由 `poll_interval_secs`（默认 45 秒）周期性复查兜底：既把最坏延迟压到该值内，又用周期性
  小流量保活长连接、避免被代理中途掐断。IDLE 推送正常送达时仍会提前唤醒，保持近乎实时。

## [0.2.1] - 2026-07-07

### 修复（健壮性 / 兜底）
- `already_running()` 存活探测加读/写超时（3 秒）：避免遇到卡死、或正处于关停窗口的旧守护
  进程时，新实例在 `read_to_string` 上永久阻塞、卡在启动无法 bind。超时即视为无健康实例并
  接管 socket（健康实例毫秒级应答，不会误伤）。
- 翻译前增加空正文前置判断：正文提取为空 / 纯空白时直接返回，不再触发本地 NLLB 引擎为“空”
  下载约 600MB 模型、或在线引擎发出无谓请求。
