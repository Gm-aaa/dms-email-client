...
### 性能
- 轮询周期加「廉价变更检测」：`examine` 响应免费带回的 `(UIDNEXT, EXISTS)` 与上轮相同（即无
  邮件增删）时，跳过随邮箱总量线性增长的 `uid_search("ALL")` 与头部抓取，仅对缓存的置顶邮件
  取一次 `FLAGS` 同步已读态。缩短轮询间隔后每轮开销不再随邮箱大小上升，与邮箱容量无关地保持
  廉价；新邮件到达（签名变化）时照常回退全量同步，行为不变。
